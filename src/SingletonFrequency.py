...
class SingletonFrequency(object):
	"""docstring for SingletonFrequency"""
	def __init__(self, arg=None, cipherText = None):
	...
	def run(self):
		...
		if len(self.possiblePositions) > 1:
			for i in self.possiblePositions: print('Possible plaintext are: ', self.frequencyMapForPlaintext[self.possiblePositions[i]])
		elif len(self.possiblePositions) == 1: print('Possible text is: ', self.frequencyMapForPlaintext[self.possiblePositions[0]])
		else: print('I failed as always, so its okay!')
		

if __name__ == "__main__":
	SingletonFrequency().run()